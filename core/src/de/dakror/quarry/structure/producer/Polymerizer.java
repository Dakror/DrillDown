 ...
 */
public class Polymerizer extends ProducerStructure {
    public static final ProducerSchema classSchema = new ProducerSchema(0, StructureType.Polymerizer, 4, 4,
            "polymerizer",
            new Items(ItemType.MachineFrame, 20, ItemType.Dynamo, 25, ItemType.SteelTube, 50, ItemType.TinPlate, 90, ItemType.Glass, 90),
            new RecipeList() {
                @Override
                protected void init() {
                    add(new Recipe(70f, "plasticbeads", 667).input(new Amount(ItemType.RefinedOil, 10000)).output(new Amount(ItemType.PlasticBeads, 80)));
                }
            }, new Sfx("polymerizer" + Const.SFX_FORMAT),
            true,
    ...
    public Polymerizer(int x, int y) {
